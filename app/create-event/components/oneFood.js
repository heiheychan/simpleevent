...
export default function OneFood({ food, innerRef, provided, setValue, removeValue, id  }) {
  return (
    <div
      ...
      ref={innerRef}
      className="w-full flex flex-row justify-between items-center border border-gray-300 h-12 rounded-full px-3 mb-2"
    >
      <div className="flex flex-row items-center">
        <MdOutlineDragIndicator size={20} />
        <input type="text" value={food} className="ml-2" onChange={setValue} id={id} placeholder="Enter the food" />
      </div>
